...
void __ISR(_TIMER_23_VECTOR, ipl5) Timer23Handler(void)
{
    ClearICTmr() ;
...
void __ISR(_INPUT_CAPTURE_1_VECTOR, ipl6) IC1Handler(void)
{
    mIC1ClearIntFlag() ;
    unsigned int i ;
    
    if ( T2CONbits.ON ) {                                                       // if Timer2 enabled
        if ( IC1CONbits.ICBNE ) {                                               // if got IC data
            if ( ICCnt < ICAvgCnt ) {
                ICTmr[ICCnt] = IC1BUF ;
                ICCnt++ ;
            } else {
                for ( i = 0 ; i < ICAvgCnt - 1 ; i++ )
                    ICTmr[i] = ICTmr[i+1] ;
                ICTmr[ICAvgCnt-1] = IC1BUF ;
            }
        }
    } else {
        ClearICTmr() ;
        T2CONbits.ON = 1 ;                                                      // enable Timer1
    }
...
{
    static unsigned char cntr1 ;
    static unsigned char cntr2 ;
...
void __ISR(_UART_1_VECTOR, IPL3) Uart1RxHandler(void)
{
    while ( UARTReceivedDataIsAvailable(USBUARTid) )
        ProcessUSBData( UARTGetData(USBUARTid).data8bit ) ;

    INTClearFlag( INT_SOURCE_UART_RX(USBUARTid) );
}